...
The optimizer suite needs a production build that exposes the Next.js
testing API, a stable URL for that build, and a Playwright command that can
drive the selected source Links. Discover this setup once, record it in
`instant-nav.rig.md`, and reuse it across optimization runs.
...
development server cannot verify the prefetched result.
...
List differences between the state used to choose the navigation contract and
